...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    static bool is_layer_1_active = false;
    switch (keycode) {
        case MC_SCREENSHOT:
        ...
        case MC_1:
            if (record->event.pressed) {
                if (is_layer_1_active) {
                    layer_move(LAYER_3);  // switch to Layer 3
                } else {
                    layer_move(LAYER_1);  // switch to Layer 1
                }
                is_layer_1_active = !is_layer_1_active;
            }
            break;
    }
    return true;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [LAYER_3] = LAYOUT_90_ansi(
        ...
        MC_5,       KC_LCTL,  KC_LWIN,            KC_LALT,  KC_SPC,   KC_DEL,              LT_SPC,             KC_RALT,            KC_RCTL,            KC_LEFT,  KC_DOWN,  KC_RGHT),
